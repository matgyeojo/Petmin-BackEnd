...
//		Review re = Review.builder().user(user).petsitter(sitter).reviewTime(3).reviewKind(4).reviewDelecacy(1)
//				.reviewMsg("아쉽네여~").build();
//		Review re2 = Review.builder().user(user).petsitter(sitter).reviewTime(2).reviewKind(5).reviewDelecacy(3)
//				.reviewMsg("아쉽네여~").build();
//		rerepo.save(re);
//		rerepo.save(re2);
//	}
...
//	void preInsert() {
//		Users user = userrepo.findById("지만").orElse(null);
//		Preference p = Preference.builder().preference1("남").preference2(20).preference3("아파트").preference4("남아")
//				.preference5("소형견").build();
//		prerepo.save(p);
//	}
//
//	// 필터링
...
//		Preference pre = Preference.builder().preference1("남").preference2(20).preference3("마당있는집").preference4("남아")
//				.preference5("소형견").user(user).build();
//
//		prerepo.save(pre);
//
//	}
...
//
//		Dolbom dol = Dolbom.builder().user2(sitter).scheduleDay("2023/08/01").scheduleHour("00:00").dolbomStatus(false)
//				.dolbomOption("단기예약").build();
//
//		dolbomrepo.save(dol);
//
//	}
...
//		PetsitterProfile sitter1 = PetsitterProfile.builder().users(u11).sitterTem(39.00).build();
//		u11.setPetsitterProfile(sitter1);
//		userrepo.save(u1);
//
//		Users u2 = Users.builder().userId("지만2").userPass("1234").userAge(25).userAddress("경기도 일산동구").userSex("남")
//				.userName("지만2").userLicence("실버펫시터").build();
//		Users u22 = userrepo.save(u2);
//		PetsitterProfile sitter2 = PetsitterProfile.builder().users(u22).sitterTem(39.00).build();
